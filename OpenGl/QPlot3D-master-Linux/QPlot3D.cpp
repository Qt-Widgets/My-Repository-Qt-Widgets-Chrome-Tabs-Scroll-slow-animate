...
void QAxis::adjustPlaneView()
{
   ...
   if (0.0 <= mPlot->elevation() && 90.0 > mPlot->elevation() ) {
      ...
      if (0.0 <= mPlot->azimuth() && 90.0 > mPlot->azimuth() ) {
         if ( mAxis == X_AXIS) {
            setVisibleTicks(true, true, false, false);
         }

         if ( mAxis == Y_AXIS) {
            setVisibleTicks(false, false, false, false);
         }

         if ( mAxis == Z_AXIS) {
            setVisibleTicks(false, false, true, false);
         }
      }

      if (90.0 <= mPlot->azimuth() && 180.0 > mPlot->azimuth() ) {
         if ( mAxis == X_AXIS) {
            setVisibleTicks(false, true, true, false);
         }
      ...
      if (180.0 <= mPlot->azimuth() && 270.0 > mPlot->azimuth() ) {
         if ( mAxis == X_AXIS) {
            setVisibleTicks(false, false, true, true);
         }
         ...
         if ( mAxis == Z_AXIS) {
            setVisibleTicks(true, false, false, false);
         }
      }

      if (270.0 <= mPlot->azimuth() && 360.0 > mPlot->azimuth() ) {
         if ( mAxis == X_AXIS) {
            setVisibleTicks(true, false, false, true);
         }

         if ( mAxis == Y_AXIS) {
            setVisibleTicks(false, false, false, true);
         }
...
void QPlot3D::drawLegend()
{
   ...
   for (int i = 0; i < nrCurves; i++) {
      ...
      if (fontMetrics().width(string) > textWidth) {
         textWidth = fontMetrics().width(string);
      }

      if (fontMetrics().height() > textHeight) {
